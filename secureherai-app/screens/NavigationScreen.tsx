...
const NavigationScreen: React.FC = () => {
  ...
  const [stableDestination, setStableDestination] = useState<MapLocation | null>(null);
  ...
  const isSignificantLocationChange = useCallback((
    oldLoc: MapLocation | null, 
    newLoc: MapLocation | null
  ): boolean => {
  ...
  const fetchParticipantLocation = useCallback(async () => {
    try {
      ...
      console.log(`📍 Fetching ${userRole === "USER" ? "responder" : "user"} location...`, result);
      ...
      if (result.success && result.participantLocation) {
        ...
        if (latitude != null && longitude != null) {
          ...
          console.warn(`📍 ${userRole === "USER" ? "Responder" : "User"} location is null - they may not have shared location yet`);
          console.log("Location data:", { latitude, longitude, lastUpdate });
          setConnectionStatus("connecting");
          // Don't clear existing location if we had one before
        }
      } else {
        console.warn("Failed to get participant location:", result.error || "Unknown error");
      ...
      if (location && location.coords && 
        ...
        try {
          ...
          if (updateResult.success) {
            ...
            console.warn("📍 Failed to update location on server:", updateResult.error);
          }
        } catch (error) {
  ...
  }, [alertId, initializeCurrentLocation, fetchParticipantLocation, stopLocationPolling, stopLocationUpdates]);
  ...
  useEffect(() => {
    ...
    if (stableOrigin && stableDestination && !mapLoaded) {
      setMapLoaded(true);
      console.log("🗺️ Initial route coordinates set - ready for manual reload only");
    }
  }, [currentLocation, targetCurrentLocation, targetLocation, stableOrigin, stableDestination, mapLoaded]);
  ...
  useEffect(() => {
    if (currentLocation && targetCurrentLocation && 
        ...
        targetCurrentLocation.latitude != null && targetCurrentLocation.longitude != null) {
  ...
  }, [fetchParticipantLocation, manualLocationReload, initializeCurrentLocation, 
      currentLocation, targetCurrentLocation, targetLocation, showAlert]);
  ...
  }, [initializeCurrentLocation, fetchParticipantLocation, 
      currentLocation, targetCurrentLocation, targetLocation, showAlert]);
  ...
  const markers = useMemo(() => {
    ...
    if (targetCurrentLocation && targetCurrentLocation.latitude != null && targetCurrentLocation.longitude != null) {
      markerList.push({
        id: "targetCurrent",
        coordinate: {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="bg-[#67082F] px-4 pt-12 pb-4">
        ...
        <View className="flex-row justify-between items-center mt-1">
          ...
          <View className="flex-row items-center">
            ...
            <Text className="text-white text-xs opacity-70 mr-3">Refresh Location</Text> */}
            ...
            <Text className="text-white text-xs opacity-70">Map Reload</Text>
          </View>
        </View>
      </View>
      ...
      <View className="bg-white border-t border-gray-200 p-4">
        ...
        {targetCurrentLocation && targetCurrentLocation.latitude != null && targetCurrentLocation.longitude != null ? (
          <View className="flex-row items-center">
            <MaterialIcons 
          ...
          <View className="flex-row items-center">
            ...
            <Text className="ml-2 text-gray-500">
              {connectionStatus === "connecting" 
                ? `Waiting for ${userRole === "USER" ? "responder" : "user"} to share location...`
                : connectionStatus === "disconnected"
                ? `Unable to connect to ${userRole === "USER" ? "responder" : "user"}`
                : `Getting ${userRole === "USER" ? "responder" : "user"} location...`
              }
            </Text>
          </View>
        )}
