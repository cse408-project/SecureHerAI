...
export default function LoginScreen() {
  ...
  const handleGoogleAuthSuccess = async (token: string) => {
    try {
      // Store the token and navigate to the app
      // You can implement your auth context logic here
      console.log("Google auth successful, token:", token);
      showAlert("Success", "Successfully signed in with Google!", "success");

      // For now, just navigate to home - you might want to save the token first
      router.replace("/");
    } catch (error) {
      console.error("Error handling Google auth success:", error);
      showAlert("Error", "Failed to complete Google authentication", "error");
    } finally {
      setShowGoogleAuth(false);
    }
  };
