...
import { Video, ResizeMode, AVPlaybackStatus, VideoFullscreenUpdateEvent, VideoFullscreenUpdate } from "expo-av";
...
export default function ReportDetailsScreen() {
  ...
  const getFileTypeFromUrl = (url: string) => {
    ...
    if (lowercaseUrl.includes('/audio/') || lowercaseUrl.includes('_audio_')) {
      return 'audio';
    } else if (lowercaseUrl.includes('/image/') || lowercaseUrl.includes('_image_')) {
      return 'image';
    } else if (lowercaseUrl.includes('/video/') || lowercaseUrl.includes('_video_')) {
      return 'video';
    } else if (lowercaseUrl.includes('/document/') || lowercaseUrl.includes('_doc_') || lowercaseUrl.includes('_document_')) {
      return 'document';
    }
  ...
  const handlePlaybackStatusUpdate = (status: AVPlaybackStatus) => {
    ...
    if (!status.isLoaded && status.error) {
      ...
      setIsPlaying(false);
      showAlert("Error", "Failed to load media. Please try opening in browser instead.", "error");
      return;
    }
  ...
  const retryMediaLoad = async () => {
    ...
    if (videoRef.current && report?.evidence && report.evidence[selectedEvidenceIndex]) {
      try {
        // Reload the video
        ...
        setHasMediaError(true);
        showAlert("Error", "Failed to reload media. Please try opening in browser.", "error");
      }
    }
  };
      ...
      <ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white rounded-lg p-4 mb-4 shadow-sm">
          <View className="flex-row items-center mb-3">
            <Text className="text-base font-semibold text-gray-800">Evidence</Text>
            {report.evidence && report.evidence.length > 0 && (
              <View className="ml-2 px-2 py-0.5 rounded-full bg-[#67082F]/10">
                <Text className="text-xs font-medium text-[#67082F]">{report.evidence.length}</Text>
              </View>
            )}
          </View>
          ...
          {report.evidence && report.evidence.length > 0 ? (
                ...
                return (
                  ...
                  >
                    <View className="w-10 h-10 rounded-lg items-center justify-center mr-3" 
                      style={{ backgroundColor: `${getFileIconColor(fileType)}15` }}>
                      <MaterialIcons
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-gray-500 text-xs capitalize">
                        {fileType} • {new Date(report.updatedAt).toLocaleDateString()}
                      </Text>
                    </View>
                    <MaterialIcons name="visibility" size={20} color="#67082F" />
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View className="p-4 bg-gray-50 rounded-lg items-center">
              ...
              <Text className="text-gray-600 text-center mt-2">No evidence attached</Text>
      ...
      >
        <View className="flex-1 bg-black/90">
          ...
          <View className="flex-1 justify-center items-center px-4">
            {report && report.evidence && report.evidence[selectedEvidenceIndex] && (
              <>
                {getFileTypeFromUrl(report.evidence[selectedEvidenceIndex]) === 'image' ? (
                  <View className="bg-gray-800 rounded-lg p-4 items-center justify-center" style={{
                ...
                ) : getFileTypeFromUrl(report.evidence[selectedEvidenceIndex]) === 'video' ? (
                      ...
                      {hasMediaError ? (
                        <View className="items-center justify-center py-8 px-4">
                          ...
                          <Text className="text-white text-center mt-4">
                            Unable to load video. Please try again or open in browser.
                          </Text>
                          <View className="flex-row mt-4 space-x-3">
                            ...
                            >
                              <MaterialIcons name="refresh" size={20} color="white" style={{marginRight: 4}} />
                              <Text className="text-white font-medium">Retry</Text>
                            </TouchableOpacity>
                            <TouchableOpacity
                              className="bg-[#3B82F6] px-4 py-2 rounded-lg flex-row items-center"
                              onPress={() => Linking.openURL(report.evidence[selectedEvidenceIndex])}
                            >
                              <MaterialIcons name="open-in-browser" size={20} color="white" style={{marginRight: 4}} />
                      ...
                      {!hasMediaError && (
                        <View className="w-full px-2 mt-2">
                          <View className="flex-row justify-between">
                            <Text className="text-white text-xs">{formatTime(playbackProgress)}</Text>
                            <Text className="text-white text-xs">{formatTime(playbackDuration)}</Text>
                          </View>
                        </View>
                      )}
                ...
                ) : getFileTypeFromUrl(report.evidence[selectedEvidenceIndex]) === 'audio' ? (
                  <View className="items-center">
                    <View className="bg-gray-800 rounded-lg p-8 items-center justify-center" style={{
                      width: Dimensions.get('window').width * 0.9,
                      ...
                      {hasMediaError ? (
                        <View className="items-center justify-center py-4">
                          <MaterialIcons name="error-outline" size={60} color="#EF4444" />
                          <Text className="text-white text-center mt-4">
                            Unable to load audio. Please try again or open in browser.
                          </Text>
                          <View className="flex-row mt-4 space-x-3">
                            <TouchableOpacity
                              className="bg-[#10B981] px-4 py-2 rounded-lg flex-row items-center"
                  ...
                  <View className="items-center">
                    <View className="bg-gray-800 rounded-lg p-8 items-center justify-center" style={{
                      ...
                      <MaterialIcons 
                        name={getFileIcon(getFileTypeFromUrl(report.evidence[selectedEvidenceIndex])) as any} 
                        size={60} 
                        color={getFileIconColor(getFileTypeFromUrl(report.evidence[selectedEvidenceIndex]))} 
                      />
                      <Text className="text-white mt-4 text-center font-medium">
                        This file type cannot be previewed directly.
                      </Text>
                      <Text className="text-white/70 text-sm mt-1 text-center">
                        {report.evidence[selectedEvidenceIndex].split('/').pop()}
                      </Text>
                      <TouchableOpacity
                        className="mt-6 bg-[#67082F] px-6 py-3 rounded-lg flex-row items-center"
                        onPress={() => Linking.openURL(report.evidence[selectedEvidenceIndex])}
                      >
                        <MaterialIcons name="open-in-browser" size={24} color="white" style={{marginRight: 8}} />
                        <Text className="text-white font-bold">Open File</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                )}
            ...
            <View className="flex-row justify-between items-center p-4">
              <TouchableOpacity
                ...
                onPress={() => selectedEvidenceIndex > 0 && setSelectedEvidenceIndex(selectedEvidenceIndex - 1)}
              ...
              <TouchableOpacity
                className="bg-gray-800 p-3 rounded-full"
                disabled={!report?.evidence || selectedEvidenceIndex >= report.evidence.length - 1}
                onPress={() => report?.evidence && selectedEvidenceIndex < report.evidence.length - 1 && setSelectedEvidenceIndex(selectedEvidenceIndex + 1)}
                style={{ opacity: !report?.evidence || selectedEvidenceIndex >= report.evidence.length - 1 ? 0.5 : 1 }}
              >
