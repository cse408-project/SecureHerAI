...
const emergencyContacts: { name: string; phone: string; icon: React.ComponentProps<typeof MaterialIcons>["name"]; }[] = [
...
const trustedContacts: { name: string; phone: string; icon: React.ComponentProps<typeof MaterialIcons>["name"]; relation: string }[] = [
...
export default function ContactsScreen() {
  ...
  const [newContact, setNewContact] = useState({ name: '', phone: '', icon: 'person' as React.ComponentProps<typeof MaterialIcons>["name"], relation: '' });
  const [contacts, setContacts] = useState(trustedContacts);
  const [selected, setSelected] = useState<number[]>([]);
  // For long press selection
  ...
  return (
    <View className="flex-1 bg-[#FFE4D6]">
      <View className="bg-[#67082F] px-4 py-6 flex-row justify-between items-center pt-12">
        ...
        <TouchableOpacity
          className="w-10 h-10 bg-red-700/30 rounded-full items-center justify-center"
      ...
      {showAddContact && (
        <View style={{ backgroundColor: 'white', margin: 16, padding: 16, borderRadius: 12, elevation: 4 }}>
          <Text style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 8 }}>Add Trusted Contact</Text>
          <View style={{ flexDirection: 'row', marginBottom: 8, alignItems: 'center' }}>
            <MaterialIcons name="person" size={24} color="#67082F" style={{ marginRight: 8 }} />
            <TextInput
              style={{ flex: 1, borderWidth: 1, borderColor: '#67082F', borderRadius: 6, padding: 6 }}
              placeholder="Name"
          ...
          <View style={{ flexDirection: 'row', marginBottom: 8, alignItems: 'center' }}>
            <MaterialIcons name="phone" size={24} color="#67082F" style={{ marginRight: 8 }} />
            <TextInput
              style={{ flex: 1, borderWidth: 1, borderColor: '#67082F', borderRadius: 6, padding: 6 }}
              placeholder="Phone"
          ...
          <View style={{ flexDirection: 'row', marginBottom: 8, alignItems: 'center' }}>
            <MaterialIcons name="group" size={24} color="#67082F" style={{ marginRight: 8 }} />
            <TextInput
              style={{ flex: 1, borderWidth: 1, borderColor: '#67082F', borderRadius: 6, padding: 6 }}
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'flex-end' }}>
            <TouchableOpacity onPress={() => setShowAddContact(false)} style={{ marginRight: 12 }}>
              <Text style={{ color: '#67082F', fontWeight: 'bold' }}>Cancel</Text>
            </TouchableOpacity>
      ...
      {selected.length > 0 && (
        <View style={{ flexDirection: 'row', justifyContent: 'flex-end', paddingHorizontal: 24, marginBottom: 8 }}>
          <TouchableOpacity onPress={handleDelete} style={{ backgroundColor: '#dc2626', borderRadius: 8, paddingVertical: 6, paddingHorizontal: 16 }}>
            <Text style={{ color: 'white', fontWeight: 'bold' }}>Delete ({selected.length})</Text>
          </TouchableOpacity>
        </View>
      )}
      <ScrollView className="flex-1 p-4" contentContainerStyle={{ paddingBottom: 100 }}>
        <Text className="text-lg font-bold text-[#67082F] mb-4">Emergency Services</Text>
        ...
        <Text className="text-lg font-bold text-[#67082F] mb-4 mt-6">Trusted Contacts</Text>
        {contacts.map((contact, index) => (
          <TouchableOpacity
            key={index}
            className="flex-row items-center bg-white p-4 rounded-lg mb-3 shadow-sm"
            style={selected.includes(index) ? { borderColor: '#dc2626', borderWidth: 2 } : undefined}
          ...
          >
            ...
            <TouchableOpacity onPress={() => handleCall(contact.phone)} style={{ marginLeft: 8 }}>
              <MaterialIcons name="phone" size={24} color="#67082F" />
            </TouchableOpacity>
            <View>
              {selected.includes(index) ? (
                ...
                <MaterialIcons name="radio-button-unchecked" size={24} color="#67082F" />
              ) : null}
            </View>
          </TouchableOpacity>
        ))}
